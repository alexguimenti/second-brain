...
## Arguments
`$ARGUMENTS` — optional: `--dry-run`, `--auto`, `--path <folder>`, `--skip <folder>` (repeatable, adds to the exclusion list on top of `.vaultignore`), `--no-vaultignore` (ignore `.vaultignore` for this run), a specific `.md` file path, or no args to scan the entire vault with confirmation.

## Constants

- Vault root: `{{VAULT_ROOT}}`
- Search scope: all `**/*.md` files under vault root
- Ignore file: `{{VAULT_ROOT}}/.vaultignore` (gitignore-syntax, optional)
...
7. **`--skip <folder>`** (repeatable) → adds folder to exclusion list, on top of `.vaultignore`
8. **`--no-vaultignore`** → skip reading `.vaultignore`; only explicit `--skip` folders apply
9. **Any argument ending in `.md`** → `scope = single_file`, `mode = confirm`

Flags can be combined freely:
- `--path Work/ClickUp/ --dry-run` is valid
- `--auto --skip Work/Recaps/` is valid (adds one more exclusion on top of `.vaultignore`)
- `--auto --no-vaultignore` is valid (full vault, ignoring the committed exclusion list)

**Build `skip_folders`:**
1. If `{{VAULT_ROOT}}/.vaultignore` exists and `--no-vaultignore` was not passed, read it (gitignore syntax, one path-prefix pattern per line, `#` comments and blank lines ignored) and add every pattern to `skip_folders`.
2. Add every `--skip <folder>` argument to `skip_folders`.
3. `skip_folders` is the union of both — deduplicate if the same folder appears in both sources.

When `scope = all`, scan the entire vault excluding `skip_folders`. Note: this exclusion list only applies to `/link-vault` — `/vault` search intentionally has no restrictions (see `docs/architecture.md` decision log).
...
2. **Exclude files** in any `skip_folders` paths (e.g., if `Work/Linear/` is in `skip_folders`, exclude `Work/Linear/**/*.md`).
...
4. Store as the **filename index** — used to detect inline text mentions.
...
- **Anything inside a markdown link — both the text and the target.** Never touch a single character between `[` and `)` of a `[text](target)` construct. This includes relative paths like `](../Claude Code/Sessions/foo.md)`.
- **Anything inside inline code spans** (between single backticks).
- **Anything that is part of a path, filename, or URL** — a token containing `/`, `\`, `.md`, `.py`, `.json`, `http://`, or `https://`. A folder or file named after a note is not a mention of that note.

> **Why this list is not negotiable.** On 2026-07-09 an `--auto` run wikilinked the word "Claude" inside 115 relative link targets in a single folder, turning `](../Claude Code/Sessions/x.md)` into `](../[[Claude]] Code/Sessions/x.md)` and killing every one of them. In one case the replacement also ate characters out of the filename (`readme-claude` → `reaclaudeaude`). The damage went unnoticed for three months. A wikilink inside a path is never correct.
...
After processing all files in scope (excluding skip_folders), display:
...
1. **Inline links:** For each proposed inline change, use Edit to replace the plain text mention with the `[[wikilink]]` version. Process one file at a time. Be precise — replace only the exact match, preserve surrounding text.
   ...
   **Before each Edit, re-read the `old_string` you are about to replace and confirm all three:**
   - it is not inside `[...](...)`, backticks, frontmatter, or a code block
   - it contains no `/`, `\`, or file extension
   - the `new_string` differs from `old_string` by exactly the added `[[` and `]]` — nothing else. If any other character changed, the edit is wrong; discard it.

   Never use `replace_all` for inline links. One occurrence per Edit, judged in its own context.
...
3. **Verify before reporting success.** For every file you modified, grep it for `\]\([^)]*\[\[` — a wikilink inside a markdown link target. Any hit is damage you just caused: revert that specific edit before reporting. Report the check either way:
...
- Files modified: N
- Path-corruption check: PASS (0 wikilinks inside link targets) | REVERTED N
```
