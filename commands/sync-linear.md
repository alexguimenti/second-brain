...
## Arguments
`$ARGUMENTS` — optional: a team key (e.g., `pulse`, `lumen`, `compass`) to sync only one team. No args = sync all tracked teams.

## Constants

- Vault root: `{{VAULT_ROOT}}`
- Output: one file per team at `Work/Linear/<TeamKey>.md`
- Teams to sync: `LUMEN`, `PULSE`, `COMPASS`
...
From `$ARGUMENTS`:
1. **No arguments** → sync all 3 teams: LUMEN, PULSE, COMPASS
2. **A team key** (e.g., `lumen`) → sync only that team (case-insensitive)
...
|------|---------------|------|
| LUMEN | <N> | Work/Linear/LUMEN.md |
| PULSE | <N> | Work/Linear/PULSE.md |
| COMPASS | <N> | Work/Linear/COMPASS.md |
...
- **Team not found:** "Team '<key>' not found. Available teams: LUMEN, PULSE, COMPASS."
