...
```
  ...
  INTELLIGENCE LOOP (weeknights, fully automatic)
  ─────────────────────────────────────────────
  Mon–Fri  Insight Agent  → reads vault (sessions, memory, chat, product docs)
  21:00                   → runs blind-spots lens via Claude
                          → saves to Vault/Work/Insights/{date}.md
                          → posts summary to ClickUp

  Mon–Fri  Research Agent → picks top 3 topics from insight
  21:15                   → fetches live data via Gemini 2.5
  ...
  19:00          Seam integrity check → daily reflection → curate MEMORY.md
  20:00          Link vault → discover document connections
  Mon–Fri 21:00  Insight Agent (blind-spots lens)
  Mon–Fri 21:15  Research Agent (Gemini + Claude)
...
5. **Register scheduled tasks:**
   ...
   **Phase 7 Intelligence Loop tasks** (Insight Agent at 21:00 + Mon 08:00, Research Agent at 21:15) are not yet auto-registered — set them up via Task Scheduler manually, pointing at `scripts/insight-agent.py` and `scripts/research-agent.py` (see Phase 7 docs below for invocation).
...
/link-vault --auto --skip Work/Recaps/  # Auto-apply, one-off extra exclusion (repeatable)
/link-vault --auto --no-vaultignore     # Full vault, ignoring .vaultignore for this run
/link-vault my-file.md             # Scan a single file
```

Persistent exclusions (chat snapshots, auto-backups, Linear snapshots — anything not worth linking) live in `<vault_root>/.vaultignore`, a gitignore-syntax file read automatically by `/link-vault` on every run. `--skip <folder>` adds a one-off exclusion on top of it for a single invocation; it doesn't edit the file. `/vault` search intentionally ignores `.vaultignore` — it has no scope restrictions by design (see `docs/architecture.md`).
...
| `/link-vault` | `/link-vault --auto --skip <folder>` | Auto-apply, one-off extra exclusion — repeatable |
| `/link-vault` | `/link-vault --auto --no-vaultignore` | Auto-apply, ignoring `.vaultignore` for this run |
...
└── Work/
    ...
    │   ├── LUMEN.md                 # Lumen team snapshot
    │   ├── PULSE.md                 # Pulse team snapshot
    │   └── COMPASS.md               # Compass team snapshot
    ├── Claude Code/
    │   └── Sessions/                # Session summaries (session)
    │       └── auto/                # Auto-backups from hook (session-auto)
    ├── EOD/                         # End-of-day summaries (eod)
    └── Company Docs/                # Company project documentation (company-docs)
```
...
QMD finds **documents** relevant to your query. LightRAG finds **entities and relationships** — it understands that "Jordan" is "Lead of Lumen" who "reported to Sam" who "decided to delay the Q3 launch." Use QMD for most searches, LightRAG when you need to understand connections.
...
- `/sync-linear` — Linear ticket snapshots per team (LUMEN, PULSE, COMPASS)
...
**What's working:**
- Seam integrity check at 19:00 (`scripts/check-seam.py`) — verifies runtime config pointers, skill junctions, hooks, and scheduled tasks haven't silently drifted or been overwritten
