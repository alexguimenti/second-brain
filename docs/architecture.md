...
| 2026-07-10 | `.vaultignore` for /link-vault scope | Was scattered as inline `--skip` flags in `scripts/daily-link-vault.sh`. One committed file (gitignore syntax, at vault root) is the single source of truth; `--skip` remains available for one-off additions. Inspired by `codebase-memory-mcp`'s `.cbmignore`. Deliberately does not apply to `/vault` — that command's "no restrictions" decision (2026-03-13, above) still stands |
| 2026-07-10 | Rejected: background watcher for real-time QMD reindex | Considered mirroring `codebase-memory-mcp`'s watcher (auto-reindex on file save instead of the fixed 07:00/13:00/20:00 schedule). Adversarial review + user confirmation: not worth it — a PM vault doesn't need near-real-time freshness, a watcher would race the existing cron jobs on the same SQLite/JSON files without a lock, needs OS-level supervision to survive reboot/sleep, and it's unverified whether QMD even supports incremental (vs. full) re-embedding. User confirmed the current staleness window has never actually caused a problem in practice |
| 2026-07-10 | Rejected: committed compressed index snapshot | Considered mirroring `codebase-memory-mcp`'s `.codebase-memory/graph.db.zst` pattern for QMD/LightRAG. Measured actual cold-rebuild cost before building anything (non-destructively — footprint + doc/chunk counts, no real rebuild triggered): QMD's real index is 27MB and rebuilds free/local (the 2.1GB "cache" is generic downloaded models, not vault data); LightRAG (233 docs, 425 chunks, gpt-4o-mini + text-embedding-3-large) estimated at ~$1-3 and ~5-10 min for a full rebuild. Both fall well under the reviewer's "<$5, <10min → not justified" threshold. The stale team-repo mirror that motivated this idea was actually caused by a dead git remote, not reindexing cost — see `SECURITY.md` for the lockstep-push fix |
