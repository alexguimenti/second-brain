...
/vault --path ClickUp/Product - Pulse   # Load all Pulse product docs
...
[1] Lumen Platform Reliability Plan
    ClickUp/Product - Lumen/Lumen Platform Reliability Plan.md
    → 4-week plan covering monitoring, alerting, and failover.
      Key sections: SLA targets, alert layers, rollback procedures.

[2] Pulse Platform Reliability Plan
    ClickUp/Product - Pulse/Pulse Platform Reliability Plan.md
    → Reliability roadmap for the Pulse product. Focuses on error handling,
      retry logic, and cache invalidation strategy.

[3] Alert Strategy
    ClickUp/Product - Lumen/Alert Strategy.md
...
| Mode | Usage | Behavior |
|------|-------|----------|
| Full scan + confirm | `/link-vault` | Scan entire vault (minus `.vaultignore`), show report, ask before applying |
...
| One-off exclusion | `/link-vault --skip <folder>` | Add an extra exclusion for this run only (repeatable) |
| Bypass ignore file | `/link-vault --no-vaultignore` | Scan the full vault, ignoring `.vaultignore` for this run |
...
`<vault_root>/.vaultignore` is a gitignore-syntax file listing paths that `/link-vault` should never scan or link — chat snapshots, session auto-backups, regenerated ticket snapshots, self-contained templates, and anything else not worth linking. It's read automatically on every `/link-vault` invocation (unless `--no-vaultignore` is passed) and is the single source of truth for this exclusion list — scheduled jobs like `scripts/daily-link-vault.sh` no longer need to pass a long list of `--skip` flags inline. `--skip <folder>` still works for a one-off addition on top of it. `/vault` search does not read this file — it has no scope restrictions by design.
