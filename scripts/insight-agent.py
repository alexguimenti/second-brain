...
VAULT = Path.home() / "Documents/Vaults/MyVault"
CLICKUP_INSIGHT_CHANNEL = "your-channel-id-here"  # e.g. a private "insights" channel
CLICKUP_WORKSPACE = "your-workspace-id-here"
ENV_FILES = [
    Path.home() / "Documents/Projects/mgmt/.env",
    Path.home() / "Documents/Vaults/MyVault/.env",
    Path.home() / "Documents/dev/.env",
]
CLAUDE_HOME = Path.home() / ".claude"
PRODUCT_MENTOR_MEMORY = (
    CLAUDE_HOME / "projects"
    / "<claude-code-project-dir-hash-for-your-product-mentor-project>"
    / "memory"
)
...
LENSES = {
    "blind-spots": """You are an analytical PM mentor reviewing the user's work.
...
4. **Neglected products** — which of {Pulse, Lumen, Compass, Beacon, Scout} got zero attention recently?
...
For each of {Pulse, Lumen, Compass, Beacon, Scout} — use 🟢/🟡/🔴 based on activity and issues.
...
def collect_product_docs(max_per_product: int = 2) -> list[tuple[str, int]]:
    ...
    # Linear snapshots (LUMEN.md, PULSE.md, COMPASS.md)
...
PM: Jordan Lee (Senior PM at Acme Corp)
Products: Pulse, Lumen, Compass, Beacon, Scout
"""
...
def get_clickup_token() -> tuple[str | None, bool]:
    """Return (token, used_fallback). Prefers SA bot token; falls back to personal."""
    token = load_env_var("CLICKUP_API_TOKEN_SA_BOT")
    if token:
        return token, False
...
def post_to_clickup(result: str, lens: str) -> None:
    ...
    token, used_fallback = get_clickup_token()
    if not token:
        print("WARNING: No ClickUp token found (tried SA bot + personal), skipping post.", file=sys.stderr)
        return
    if used_fallback:
        print("WARNING: SA bot token unavailable, falling back to personal token.", file=sys.stderr)
    ...
    fallback_banner = "⚠️ _Posted via fallback — SA bot token unavailable_\n\n" if used_fallback else ""
