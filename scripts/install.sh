...
# 7. Install git pre-commit guard (blocks secrets + the ID->name roster shape)
echo ""
echo "Installing git pre-commit guard..."

SAFETY_CONFIG="$REPO_ROOT/scripts/repo-safety.config.json"
if [ ! -f "$SAFETY_CONFIG" ]; then
  cp "$REPO_ROOT/scripts/repo-safety.config.example.json" "$SAFETY_CONFIG"
  echo "  Created scripts/repo-safety.config.json from template — fill in your"
  echo "  own company/product/personal terms (see the file's _comment)"
else
  echo "  scripts/repo-safety.config.json already exists, skipping"
fi
...
#!/usr/bin/env bash
exec "$REPO_ROOT/scripts/check-repo-safe.sh"
HOOK
  chmod +x "$GIT_HOOKS_DIR/pre-commit"
  echo "  Installed $GIT_HOOKS_DIR/pre-commit -> scripts/check-repo-safe.sh"
else
  echo "  No .git/hooks directory found — skipping (not a git repo?)"
fi
